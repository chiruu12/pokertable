"""Test poker agent via Smolagents (HuggingFace) framework.

Smolagents is a lightweight agent framework that supports code-based
and tool-calling agents with various LLM backends via LiteLLM.

Requires: ANTHROPIC_API_KEY or OPENAI_API_KEY environment variable
          pip install smolagents

Usage: uv run python examples/llm-integration-tests/test_smolagents.py
"""
...
try:
    ...
    print("SKIP: smolagents not installed — pip install smolagents")
    sys.exit(0)
...
from frameworks.smolagents_adapter import SmolagentsAdapter  # noqa: E402

NUM_HANDS = 3


class SmolagentsPlayer:
    """Wraps SmolagentsAdapter into a player."""

    def __init__(self, name: str, adapter: SmolagentsAdapter) -> None:
        self._name = name
...
async def main() -> None:
    ...
    print(f"Smolagents Framework Test — {model} vs Random Bot")
    print(f"Playing {NUM_HANDS} hands\n")

    engine = PokerEngine(["Smol-Agent", "RandomBot"], starting_chips=500, seed=42)
    toolkit = PokerToolkit(engine, "Smol-Agent")

    adapter = SmolagentsAdapter(toolkit=toolkit, model=model)
    smol_player = SmolagentsPlayer("Smol-Agent", adapter)
    random_player = RandomPlayer("RandomBot", seed=1)

    players = {"Smol-Agent": smol_player, "RandomBot": random_player}
